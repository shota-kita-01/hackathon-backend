...
def run_experiment():
    ...
    print("🧠 Imagen 3 モデル（最新安定版）をロード中...")
    model = ImageGenerationModel.from_pretrained("imagen-3.0-generate-002")
    
    # デモ用のプロンプト（フリマアプリで映える最高画質のスニーカー）
    prompt = (
    ...
    print("⏳ Imagen 3 で画像を生成しています（数秒かかります）...")
    try:
        response = model.generate_images(
            prompt=prompt,
            number_of_images=1,
            aspect_ratio="1:1",  # フリマに最適な正方形
            language="en",
        )
        
        if response.images:
            # 💡 scriptsフォルダと同じ階層（プロジェクトのルート）に保存されるようパスを設定
            ...
            print(f"🎉 成功しました！画像が生成され、以下に保存されました:\n👉 {os.path.abspath(output_path)}")
        else:
            print("❌ 画像の生成結果が空でした。")
            
    except Exception as e:
        print(f"❌ エラーが発生しました: {str(e)}")
        print("💡 認証が通っていない場合は、事前に『gcloud auth application-default login』を実行してください。")
