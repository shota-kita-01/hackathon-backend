...
def process_single_item(item):
    ...
    try:
        ...
        # 安全に翻訳後の日本語テキストをパース
        name_ja = parsed_json.get("name_ja") if hasattr(parsed_json, 'get') else None
        description_ja = parsed_json.get("description_ja") if hasattr(parsed_json, 'get') else None
        category = item.get('ai_category', '')
        
        # 🛠️ 【数理調停】英語テンプレートを撤廃し、ピュアな日本語結合テキストへ統合
        structured_text = f"{name_ja} {category} {description_ja}"
        ...
        embed_res = client.models.embed_content(
            ...
            config=types.EmbedContentConfig(
                ...
                task_type="RETRIEVAL_DOCUMENT"  # 💡 ドキュメント用として空間の次元を固定
            )
        )
        ...
        with counter_lock:
            progress_counter += 1
            print(f"⚡ [{progress_counter}/{total_count}] ASIN: {item['asin']} ➔ ✨ 日本語空間マッピング完了！", flush=True)
...
if TEST_MODE:
    ...
    # 💡 【安全装置】既存ファイルを破壊・上書きしないよう、別名で新規書き出しします
    output_filename = "items_with_embeddings_all_2200_ja.json"
else:
    output_filename = f"items_with_embeddings_day_{CURRENT_BATCH_CYCLE}_ja.json"
